...
export default function Skills() {
    ...
    return (
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true, amount: 0.1 }} transition={{ duration: 0.3 }}>
            ...
            <motion.h2 initial={{ opacity: 0, x: -16 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, amount: 0.5 }} transition={{ duration: 0.2 }} className="text-xl font-bold mb-4">Skills</motion.h2>
            ...
            <motion.section initial={{ opacity: 0, x: -16 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, amount: 0.5 }} transition={{ duration: 0.2 }} className="flex bg-neutral-800 px-4 py-4 rounded-lg shadow-md w-full justify-center items-center">
                <section id="skill-grid" className="gap-8 w-full">
                    {skillsSelected.map((skill, index) => (
                        <motion.article initial={{ opacity: 0, x: -8 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, amount: 0.5 }} transition={{ duration: 0.25 }}
                            key={index}
