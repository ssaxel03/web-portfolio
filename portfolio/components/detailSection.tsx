...
export default function Details() {
    ...
    return (
        <motion.section initial={{opacity: 0}} whileInView={{opacity: 1}} viewport={{ once: true, amount: 0.15 }} transition={{ duration: 0.3 }} className='flex flex-col w-full lg:w-1/2'>
            ...
            <motion.h1 initial={{ opacity: 0, x: -16 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, amount: 0.15 }} transition={{ duration: 0.2, delay: 1 * 0.2}} 
            className="text-xl mb-1">{details.name}</motion.h1>

            <motion.article initial={{ opacity: 0, x: -16 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, amount: 0.15 }} transition={{ duration: 0.2, delay: 2 * 0.2}} 
            className="text-xl mb-1">{details.role}</motion.article>

            <motion.article initial={{ opacity: 0, x: -16 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, amount: 0.15 }} transition={{ duration: 0.2, delay: 3 * 0.2}}  
